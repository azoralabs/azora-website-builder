...
class CssEmitterTest {
    ...
    fun `button height and border win over az-button base via boosted selector`() {
        ...
        val css = CssEmitter.fileCss(btn, emptyMap())
    ...
    fun `button jsx carries the per-node className that the css targets`() {
        ...
        val jsx = buildSource { JsxEmitter.emit(this, btn, emptyMap(), emptyMap()) }
    ...
    fun `non-button nodes use the plain per-node selector`() {
        val col = WebColumn(
            ...
            children = listOf(WebText(id = "c_t", text = "hi", modifier = WebModifier(width = 100)))
        )
        val css = CssEmitter.fileCss(col, emptyMap())
    ...
    fun `text and link are inline-block so box modifiers apply`() {
        ...
        val css = CssEmitter.fileCss(txt, emptyMap())
    ...
    fun `all modifier fields emit when set`() {
        val col = WebColumn(
            id = "c_x",
            arrangement = dev.azora.website.builder.domain.WebArrangement.CENTER,
            modifier = WebModifier(
                ...
                fontWeight = dev.azora.website.builder.domain.WebFontWeight.BOLD,
                textAlign = dev.azora.website.builder.domain.WebTextAlign.CENTER,
